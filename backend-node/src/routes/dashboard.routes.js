...
router.get('/', asyncHandler(async (req, res) => {
  const [productsResult, salesResult, settingsResult] = await Promise.all([
  ...
  const totalRevenue = sales.reduce((sum, s) => sum + Number(s.preco_unitario) * Number(s.quantidade), 0);
  const totalOrders = sales.reduce((sum, s) => sum + Number(s.quantidade), 0);
  ...
  res.json({
    ...
    vendas_recentes: sales.slice(0, 5).map(s => {
      const product = products.find(p => p.id === s.produto_id);
      return {
        ...s,
        produto_nome: product?.nome || s.produto_nome
      };
    })
  });
}));
