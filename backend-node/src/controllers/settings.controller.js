...
async function update(req, res) {
  ...
  const existing = await query('SELECT id FROM configuracoes LIMIT 1');
  ...
  if (existing.rows.length === 0) {
    result = await query(
      ...
      [custo_kwh, margem_lucro_padrao, potencia_impressora_w, estoque_baixo_limite]
    );
  } else {
    result = await query(
      `UPDATE configuracoes SET custo_kwh = $1, margem_lucro_padrao = $2, potencia_impressora_w = $3, estoque_baixo_limite = $4
       WHERE id = $5 RETURNING *`,
      [custo_kwh, margem_lucro_padrao, potencia_impressora_w, estoque_baixo_limite, existing.rows[0].id]
    );
  }
